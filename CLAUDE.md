...
npm test           # vitest unit tests (pricing + WhatsApp links)
npm run test:watch # vitest in watch mode
npm run test:e2e   # playwright E2E + axe-core a11y (needs a server)
npm run typecheck  # astro check
```

Run a single unit test: `npx vitest run src/data/pricing.test.ts` (or `-t "<name>"` to filter).
...
Pricing, sizes, discounts, services, towns, business identity, and FAQ live **only** here. The price list (cjenik), the coverage map, and the JSON-LD all read the same data, so numbers cannot drift apart (a real past bug: home 45€ vs city 40€). Change a price in `src/data/` and it changes everywhere.

- `services.ts` / `pricing.ts` — prices, sizes, travel zones, minimum order. The travel/minimum-order model: named local towns are free, Bjelovar/Koprivnica add `SURCHARGE_BJEL_KOP`, Zagreb-east towns add `SURCHARGE_ZAGREB_EAST` with a higher `MIN_ORDER_ZAGREB_EAST` floor. Unknown places: confirm on WhatsApp.
...
- `sections/` — static page sections (Hero, Services, Timeline, About, FaqList, ContactCTA, Nav, Footer, PriceCard, …).
- `islands/` — the only client-side JS: `BeforeAfter` (slider), `Coverage` (Leaflet map), `MobileNav`.

`pages/` has `index.astro`, `podrucje-pokrivenosti.astro`, and city pages, composed in `layouts/BaseLayout.astro` (head, self-hosted fonts, JSON-LD).
